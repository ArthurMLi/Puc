            ...
            try {
                switch (command) {
                    case "II":
                        ...
                        filaDeGames.inserirInicio(game);
                        break;
                    case "IF":
                        game = findGameById(parts[1]);
                        filaDeGames.inserirFim(game);
                    ...
                    case "I*":
                        ...
                        game = findGameById(parts[2]);
                        filaDeGames.inserir(game, posI);
                        break;
                    case "RI":
                        removido = filaDeGames.removerInicio();
                    ...
                    case "R*":
                        ...
                        removido = filaDeGames.remover(posR);
                        System.out.println("(R) " + removido.getName());
                        break;
                }
            } catch (Exception e) {
...
class Fila {
    ...
    /**
     * Insere em uma posição. 0=Início, tamanho=Fim.
     */
    public void inserir(Game game, int p) {
        ...
        if (p == 0) {
            inserirInicio(game); // Reutiliza e já incrementa o tamanho
        }
        // 3. Caso Fim (p = tamanho)
        else if (p == this.tamanho) {
            inserirFim(game); // Reutiliza e já incrementa o tamanho
        }
        // 4. Caso Meio (0 < p < tamanho)
        else {
            Game tmp = primeiro;
            // Percorre até o nó ANTERIOR (p-1)
            for (int i = 0; i < p - 1; i++) {
                tmp = tmp.prox;
            }
            game.prox = tmp.prox;
            tmp.prox = game;
            this.tamanho++; // Incrementa APENAS no caso Meio
        }
    }
    ...
    /**
     * Remove de uma posição. 0=Início, (tamanho-1)=Fim.
     */
    public Game remover(int p) {
        ...
        // 3. Caso Fim (p = tamanho - 1)
        else if (p == this.tamanho - 1) {
            removido = removerFim();
        }
        // 4. Caso Meio (0 < p < tamanho - 1)
        else {
            Game tmp = primeiro; // tmp será o nó ANTERIOR (p-1)
            for (int i = 0; i < p - 1; i++) {
                tmp = tmp.prox;
            }
            removido = tmp.prox;
