...
Com base na literatura revisada, espera-se que os solvers GPU apresentem speedups relevantes em instâncias de grande porte com alto grau de paralelismo estrutural — especialmente nas fases de propagação unitária e simplificação \cite{kirk:2016}. Contudo, a eficiência energética deve ser avaliada com cautela: a GPU opera com TDP tipicamente 2 a 5 vezes superior ao da CPU, o que pode anular o ganho de desempenho em termos de Joules/instância para problemas de pequena escala \cite{mittal:2015}.

Instâncias industriais, por sua estrutura correlacionada e fluxo de controle irregular, tendem a favorecer as heurísticas CDCL da CPU, cujos Branch Predictors e caches profundas lidam bem com esse padrão de acesso \cite{hennessy:2017}. Instâncias aleatórias próximas à fase crítica — que exigem exploração ampla e pouco correlacionada do espaço de busca — podem se beneficiar do paralelismo massivo da GPU, especialmente na propagação de unidades em larga escala. Essa hipótese, se confirmada, sustenta a proposta de uma estratégia de seleção de hardware baseada em características estruturais da instância, análoga ao portfólio do SATzilla \cite{arora:2009}, mas orientada à alocação energeticamente eficiente de recursos computacionais.

O pipeline híbrido CPU-GPU — evidenciado pelo ParaFrost no CAV 2021 — representa um cenário de particular interesse: ao descarregar a simplificação massiva para a GPU e reservar o núcleo CDCL para a CPU, é possível que essa arquitetura apresente não apenas speedup, mas também melhor eficiência energética do que a GPU atuando como resolvedor integral. Medir essa diferença com precisão, via integral de potência em tempo real, é um dos grandes diferenciais empíricos deste trabalho.

\section{Conclusão}

Este trabalho apresentou uma análise comparativa entre arquiteturas CPU e GPU para a resolução do problema SAT, com foco simultâneo em desempenho e eficiência energética. A revisão de seis trabalhos de alto impacto revelou que, embora os avanços em solvers GPU sejam significativos — em especial no inprocessing paralelo (ParaFrost), no mapeamento contínuo (FastFourierSAT) e na paralelização do DPLL (CUD@SAT) —, a dimensão energética permanece sistematicamente negligenciada na literatura. A abordagem do SATzilla demonstra que a estrutura da instância é o fator determinante na seleção do melhor algoritmo; este trabalho estende esse princípio para a dimensão do hardware, propondo que a mesma lógica pode guiar a alocação eficiente entre CPU e GPU.

A metodologia proposta, baseada em perfilamento em tempo real via RAPL e NVML e no cálculo da integral de potência por instância, fornece uma abordagem rigorosa para estabelecer a razão Joules/instância e definir critérios de seleção de hardware conscientes do contexto e do custo energético. Os resultados contribuirão para orientar a alocação inteligente de recursos em data centers e para o avanço da área de \textit{Green Computing} aplicada a problemas de lógica combinatória, preenchendo uma lacuna concreta e relevante da literatura atual.
