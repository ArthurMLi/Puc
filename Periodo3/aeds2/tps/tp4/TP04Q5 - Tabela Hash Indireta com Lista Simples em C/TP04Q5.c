...
colecao_restaurantes_hash *ler_entradas(colecao_restaurantes *base)
{
    colecao_restaurantes_hash *selecionados = (colecao_restaurantes_hash *)malloc(sizeof(colecao_restaurantes_hash));
    if (selecionados == NULL)
    {
        return NULL;
    }

    selecionados->tamanhoLista = 31;
    selecionados->lista = (restaurante **)calloc(31, sizeof(restaurante *));
    selecionados->tamanhoReserva = 19;
    selecionados->reserva = (restaurante **)calloc(19, sizeof(restaurante *));
    int id, ASCIISomado;
    while (scanf("%d", &id) == 1 && id != -1)
    {
        ...
        ASCIISomado = somar_ascii(item->nome) % 31;
        if (selecionados->lista[ASCIISomado] == NULL)
        {
            selecionados->lista[ASCIISomado] = item;
        }
        else
        {
            // inserirReserva
            int i = 0;
            for (; i < 19 && selecionados->reserva[i] != NULL; i++)
            {
            }
            if (i < 19)
            {
                selecionados->reserva[i] = item;
            }
            else
            {
                printf("%s\n", item->nome);
...
int main()
{
    ...
    colecao_restaurantes_hash *hash = ler_entradas(base);
    ...
    {
        // essas 3 linahs foram o gemini me ajudando a achar o problema
        // 1. Limpa \r e \n instantaneamente
        s[strcspn(s, "\r\n")] = '\0';
        ...
        char restaurante[2048];
        int pos = somar_ascii(s) % hash->tamanhoLista;
        ...
        {
            if (strcmp(hash->lista[pos]->nome, s) == 0)
            {
                formatar_restaurante(hash->lista[pos], restaurante);
                printf("%d %s", pos, restaurante);
            }
            else
            {
                int i = 0;
                for (; i < 19 && hash->reserva[i] != NULL; i++)
                {
                    if (strcmp(hash->reserva[i]->nome, s) == 0)
                    {
                        formatar_restaurante(hash->reserva[i], restaurante);
                        printf("%d %s", (hash->tamanhoLista + i), restaurante);
                        break;
                    }
                }
                if (i == 19 || hash->reserva[i] == NULL)
                {
                    printf("-1\n");
                }
            }
