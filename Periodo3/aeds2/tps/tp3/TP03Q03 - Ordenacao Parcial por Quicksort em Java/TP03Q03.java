...
public class TP03Q03 {
    ...
    public static void main(String args[]) {
        try (Scanner sc = new Scanner(System.in)) {
            ...
            base.lerCsv("../../tp2/restaurantes.csv");
            ...
            selecionados.quicksort(10);
            long fim = System.nanoTime();
            double tempoMs = (fim - inicio) / 1_000_000.0;
            for (int i = 0; i < selecionados.tamanho; i++) {
                System.out.println(selecionados.restaurantes[i].formatar());
            }
            escreverLog(selecionados.comparacoes, selecionados.movimentacoes, tempoMs, "_sequencial_parcial.txt");
        }
    }
    ...
    static class ColecaoRestaurantes {
        ...
        public void quicksort( int k) {
            quicksort(0, tamanho - 1, k);
        }

        private void quicksort(int esq, int dir,int k) {
            int i = esq, j = dir;
            String pivo = restaurantes[(esq+dir)/2].nome;
            while (i <= j) {
                while (restaurantes[i].nome.compareTo(pivo) > 0) {
                    comparacoes++;
                    i++;
                }
                while (restaurantes[j].nome.compareTo(pivo) < 0) {
                    comparacoes++;
                    j--;
                }
        ...
        public void selecaoColecaoRestaurantes(int k) {

            for (int i = 0; i < k; i++) {
                int menoridx = i;
                for (int j = (i + 1); j < tamanho; j++) {
                    comparacoes++;
                    if (restaurantes[j].nome.compareTo(restaurantes[menoridx].nome) < 0) {
                        menoridx = j;
                    }
                }
                swap(menoridx, i);
