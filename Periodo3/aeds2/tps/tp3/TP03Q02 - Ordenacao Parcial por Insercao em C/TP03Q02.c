...
void insercaoPorCidade(colecao_restaurantes *colecao, long long *comparacoes, long long *movimentacoes, int k)
{
    for (int i = 1; i < k && i < colecao->tamanho; i++)
    {
        ...
        {
            comparacoes++;
            if (strcmp(colecao->restaurantes[j]->cidade, tmp->cidade) > 0)
            {
    ...
    for (int i = k; i < colecao->tamanho; i++)
    {
        ...
        if (strcmp(colecao->restaurantes[i]->cidade, colecao->restaurantes[k-1]->cidade) < 0)
        {
            ...
            restaurante *tmp = colecao->restaurantes[i];
            colecao->restaurantes[i] = colecao->restaurantes[k - 1];
            movimentacoes++;
