...
void lista_inserir_inicio(lista *l, restaurante *r) {
    no *n = criar_no(r);
    n->prox = l->inicio;
    l->inicio = n;
    l->tamanho++;
...
void lista_inserir_fim(lista *l, restaurante *r) {
    no *n = criar_no(r);
    if (l->inicio == NULL) {
        l->inicio = n;
    } else {
...
void lista_inserir_pos(lista *l, restaurante *r, int pos) {
...
restaurante *lista_remover_inicio(lista *l) {
    if (l->inicio == NULL) {
        return NULL;
    }
    no *aux = l->inicio;
...
restaurante *lista_remover_fim(lista *l) {
    if (l->inicio == NULL) {
        return NULL;
    }
...
restaurante *lista_remover_pos(lista *l, int pos) {
    if (l->inicio == NULL || pos < 0 || pos >= l->tamanho) {
        return NULL;
    }
...
void lista_mostrar(lista *l) {
    ...
    while (aux != NULL) {
        char s[2048];
        formatar_restaurante(aux->dado, s);
        printf("%s", s);
        aux = aux->prox;
    }
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (strcmp(cmd, "II") == 0) {
            ...
            int pos, novo_id;
            scanf("%d %d", &pos, &novo_id);
            restaurante *r = buscar_por_id(base, novo_id);
            if (r != NULL) {
                lista_inserir_pos(&l, r, pos);
            }
        } else if (strcmp(cmd, "RI") == 0) {
            restaurante *r = lista_remover_inicio(&l);
            if (r != NULL) {
                printf("(R)%s\n", r->nome);
            }
        } else if (strcmp(cmd, "RF") == 0) {
            restaurante *r = lista_remover_fim(&l);
            if (r != NULL) {
                printf("(R)%s\n", r->nome);
            }
            ...
            int pos;
            scanf("%d", &pos);
            restaurante *r = lista_remover_pos(&l, pos);
            if (r != NULL) {
                printf("(R)%s\n", r->nome);
            }
        }
