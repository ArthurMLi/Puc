...
void counting_sort_por_capacidade(colecao_restaurantes *colecao, long long *comparacoes, long long *movimentacoes) {
    if (colecao->tamanho <= 1) {
        return;
    }

    int max_cap = colecao->restaurantes[0]->capacidade;
    for (int i = 1; i < colecao->tamanho; i++) {
        (*comparacoes)++;
        if (colecao->restaurantes[i]->capacidade > max_cap) {
            max_cap = colecao->restaurantes[i]->capacidade;
        }
    }

    int *count = (int *) calloc((size_t) (max_cap + 1), sizeof(int));
    restaurante **ordenado = (restaurante **) malloc((size_t) colecao->tamanho * sizeof(restaurante *));
    if (count == NULL || ordenado == NULL) {
        free(count);
        free(ordenado);
        return;
    }
    ...
    for (int i = colecao->tamanho - 1; i >= 0; i--) {
        int cap = colecao->restaurantes[i]->capacidade;
        int pos = count[cap] - 1;
        ordenado[pos] = colecao->restaurantes[i];
        count[cap]--;
        (*movimentacoes)++;
    }
...
int main() {
    ...
    counting_sort_por_capacidade(selecionados, &comparacoes, &movimentacoes);
    clock_t fim = clock();
